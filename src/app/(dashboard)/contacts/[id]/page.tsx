      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
        <section className="space-y-5">
          <div className="card p-6">
            <div className="mb-5 flex items-start justify-between gap-4"><div><h2 className="text-2xl font-black text-white">{contact.business}</h2><p className="text-slate-400">{contact.contact || "No contact name"}</p></div><div className="flex items-center gap-2"><span className="badge">{contact.status}</span><form action={deleteContact}><input type="hidden" name="leadId" value={contact.id} /><button type="submit" className="rounded-lg border border-red-300/30 bg-red-400/10 px-3 py-1.5 text-xs font-black text-red-200">Delete</button></form></div></div>
